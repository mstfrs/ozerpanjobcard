...
import { getTesDetayDetails,getPozDetails, getAllBarcodesOfPoz } from '../../../services/TesDetayServices';
...
const KanatHazirlik = () => {
    ...
    const {
        ...
        setCurrentBarkod,
        currentBarkod,
        setIsAccessoryLoading,setMaxSanalAdet,maxSanalAdet
    } = useJobcardsStore();
    ...
    const handleBarkodChange = async (e) => {
        ...
        const fetchBOMDetails = async () => {
            ...
            try {
                ...
                const maxAdet = Math.max(...allBarcodesOfPoz.map(detail => parseInt(detail.sanal_adet, 10)));
                setMaxSanalAdet(maxAdet);


                // Item grubu "Aksesuar" olanları filtrele
                const aksesuarItems = itemDetails.filter(item => item.item_group === "Pvc Hat1 Aksesuarlar");
    ...
    return (
        <>
            <InputText  className='border-2 border-red-400 w-2/3 text-center text-xl font-semibold  mx-auto my-1 py-1' value={currentBarkod} onChange={(e) => handleBarkodChange(e)} />

            {
                currentBarkod !== undefined ? <div className='w-full flex justify-between h-[calc(100vh-100px)]  px-3 py-2'>
                    ...
                    <div className="w-1/4 h-full p-4 grid grid-cols-1 gap-4 justify-center place-items-center bg-slate-200 overflow-auto">
                        <img src="/files/aksesuar.jpg" alt="" className=' h-full w-full' />
