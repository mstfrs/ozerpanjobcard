  ...
  return (
    <div className='flex justify-between w-full gap-4'>
      <div className='w-full flex justify-start'>
        <Dropdown value={currentOperation} onChange={(e) => handleOperationChange(e)} options={employee?.custom_operations} optionLabel="operations"
          placeholder="Operasyon Seçiniz" className="w-60 md:w-14rem border rounded" />
        {
          currentOperation?.operations === "Profil Temin" || currentOperation?.operations === "Sac Kesim" ?
            <Dropdown value={currentOpt} onChange={(e) => handleOptiChange(e)} options={jobCardList?.filter(item => item.status !== "Completed")} optionLabel="custom_opti_no"
              placeholder="Opt No Seçiniz" className="w-60 md:w-20rem border rounded " /> :null
      ...
      <div className='w-full flex items-center gap-1'>
        ...
        </div>
       {
        ...
        <div onClick={handlelogOut} className='flex justify-between border-2 items-center w-36 h-12 p-1 rounded-md cursor-pointer hover:bg-red-200'>
